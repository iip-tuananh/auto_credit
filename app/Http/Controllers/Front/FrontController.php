...
use App\Model\Admin\LoanApplication;
...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        PostService::attachAccess($popularPosts, auth('customer')->id());

        // bài viết mới nhất
        $postsRecent = Post::query()->with(['category', 'image'])->where('status', 1)->latest()->limit(6)->get();
        PostService::attachAccess($postsRecent, auth('customer')->id());
        ...
        $categoriesSpecial->each(function ($cat) {
            ...
            $posts = PostService::attachAccess($posts, auth('customer')->id());
        ...
        // start
        $achievements = Achievement::query()->with(['image'])->orderBy('sort')->get();
        $feedbacks = Review::query()->with(['image'])->latest()->get();
        $questions = Question::query()->latest()->get()->take(6);
        $partners = Partner::query()->with(['image'])->latest()->get();
        $about = About::getDataForEdit(1);
        $banners = Banner::query()->whereNull('type')->with(['image'])->latest()->first();

        return view('site.home', compact('postCategories', 'popularPosts','postsRecent','categoriesSpecial', 'about', 'achievements',
            'feedbacks', 'tags', 'banners', 'questions', 'partners', 'banners'));
    }
    ...
    public function submitApply(Request $request)
    {
        ...
        try {
            $lastId = LoanApplication::query()->latest('id')->first()->id ?? 1;

            $fields = $request->extra_fields;

            $obj = new LoanApplication();
            $obj->code = 'HS' . date('Ymd') . '-' . $lastId;
            $obj->vehicle_type = @$fields['vehicle_type'] ?? '';
            $obj->vehicle_condition = @$fields['vehicle_condition'] ?? '';
            $obj->vehicle_found = @$fields['vehicle_found'] ?? '';
            $obj->driving_license = @$fields['driving_license'] ?? '';
            $obj->fullname = @$fields['fullname'];
            $obj->dob_d = @$fields['dob_d'] ?? '';
            $obj->dob_m = @$fields['dob_m'] ?? '';
            $obj->dob_y = @$fields['dob_y'] ?? '';
            $obj->dob_date = Carbon::createFromFormat('d/m/Y', $fields['dob'])->format('Y-m-d');
            $obj->marital_status = @$fields['marital_status'] ?? '';
            $obj->address_thuongtru = @$fields['address_thuongtru'] ?? '';
            $obj->address_sinhsong = @$fields['address_sinhsong'] ?? '';
            $obj->years_cutru = @$fields['years_cutru'] ?? '';
            $obj->months_cutru = @$fields['months_cutru'] ?? '';
            $obj->housing_status = @$fields['housing_status'] ?? '';
            $obj->employment_status = @$fields['employment_status'] ?? '';
            $obj->job_title = @$fields['job_title'] ?? '';
            $obj->company_name = @$fields['company_name'] ?? '';
            $obj->company_address = @$fields['company_address'] ?? '';
            $obj->work_years = @$fields['work_years'] ?? '';
            $obj->work_months = @$fields['work_months'] ?? '';
            $obj->net_income = @$fields['net_income'] ?? '';
            $obj->borrow_amount = @$fields['borrow_amount'] ?? '';
            $obj->pay_amount = @$fields['pay_amount'] ?? '';
            $obj->customer_name = @$fields['customer_name'] ?? '';
            $obj->phone = @$fields['phone'] ?? '';
            $obj->email = @$fields['email'] ?? '';
            $obj->cccd = @$fields['cccd'] ?? '';
            $obj->title = @$fields['title'] ?? 'Mr';

            $obj->save();

            session(['loan_id' => $obj->id]);
            \Illuminate\Support\Facades\DB::commit();

            $client = new \GuzzleHttp\Client();
            $googleSheetUrl = 'https://script.google.com/macros/s/AKfycbyoh8ZVRLWjYYQlgaKlfuDLkvPEdmm0yHBp35meatUWXMK6U2v0jKhQlRe1-s8l_F1h/exec';

            $customer_info = [];

            if (!empty($obj->customer_name)) {
                $customer_info[] = trim($obj->customer_name);
            }
            if (!empty($obj->cccd)) {
                $customer_info[] = 'CCCD: ' . trim($obj->cccd);
            }
            if (!empty($obj->dob_date)) {
                $customer_info[] = 'Ngày sinh: ' . Carbon::parse($obj->dob_date)->format('d/m/Y');
            }
            $customer_info = implode(', ', $customer_info); // ví dụ: "Nguyễn Văn A, CCCD: 0310..., Ngày sinh: 12/01/1997"

            $response = $client->post($googleSheetUrl, [
                'form_params' => [
                    'code'    => $obj->code,
                    'customer_info'    => $customer_info,
                    'address_sinhsong' => $obj->address_sinhsong,
                    'years_cutru' => $obj->years_cutru,
                    'months_cutru' => $obj->months_cutru,
                    'employment_status'   => $obj->employment_status,
                    'job_title'   => $obj->job_title,
                    'work_years'   => $obj->work_years,
                    'work_months'   => $obj->work_months,
                    'net_income'   => $obj->net_income,
                    'borrow_amount'   => $obj->borrow_amount,
                    'pay_amount'  => $obj->pay_amount,
                    'vehicle_type'  => $obj->vehicle_type,
                    'vehicle_condition'  => $obj->vehicle_condition,
            ...
            return \Illuminate\Support\Facades\Response::json(['success' => true, 'message' => 'thao tác thành công']);
    ...
    public function submitApplySuccess(Request $request)
    {
        if (!session()->has('loan_id')) {
            return redirect()->route('front.home-page');
        }

        $orderId = session('loan_id') ?? 39;
        $order = LoanApplication::query()->find($orderId);

        session()->forget('loan_id');
        return view('site.apply_success', compact('order'));
    }
    ...
    public function abouts(Request $request) {
        ...
        return view('site.about_us', compact('banner', 'about', 'feedbacks'));
    }
    ...
    public function faq(Request $request) {
        ...
        return view('site.faq', compact('questions', 'banner'));
    }
    ...
    public function getPostByTag(Request $request, $slug) {
        ...
        return view('site.blog_tag', compact('posts', 'tag', 'categories', 'tags', 'popularPosts', 'postsRecent'));
    }
    ...
    public function blogs(Request $request, $slug = null) {
        ...
        if($slug) {
            ...
            if(! $category->parent_id) {
                ...
                // nếu $category là danh mục con (cấp2)
                if($category->level == 1) {
                    $lv3Ids = PostCategory::query()
                        ->where('parent_id', $category->id)
                        ->pluck('id');
                    ...
                    // nếu $category là danh mục con (cấp3)
                    $cateIds = [$category->id];
                    $parentCateLv2 = PostCategory::query()->where('id', $category->parent_id)->first();
                    $parentCateLv1 = PostCategory::query()->where('id', $parentCateLv2->parent_id)->first();
    ...
    public function blogDetail(Request $request, $slug) {
        $blog = Post::query()->with(['image', 'user_create', 'category'])->where('slug', $slug)->first();
        ...
        $blog->canAccess = PostService::canAccessPost($blog);
    ...
    public function clearData() {
        PostCategory::whereColumn('parent_id', 'id')->update(['parent_id' => 0]);
    }
}
